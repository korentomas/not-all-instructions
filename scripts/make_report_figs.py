...
# fig6 — v1 -> v2 per-decision comparison (dumbbell): what changed
sv1 = summ(V1, "beta", "decision")
sv2 = summ(V2, "beta", "decision")
order2 = sv2["mean"].sort_values().index
fig, ax = plt.subplots(figsize=(8.6, 5.2))
for yi, d in enumerate(order2):
    m1, m2 = sv1.loc[d, "mean"], sv2.loc[d, "mean"]
    ax.plot([m1, m2], [yi, yi], color=RULE, lw=1.5, zorder=1)
    ax.scatter(m1, yi, color=CLAY, s=46, zorder=2, label="v1" if yi == 0 else "")
    ax.scatter(m2, yi, color=TEAL, s=46, zorder=2, label="v2 (M4)" if yi == 0 else "")
ax.axvline(0, color=MUTED, lw=1, ls=(0, (4, 3)), alpha=.6)
ax.set_yticks(range(len(order2))); ax.set_yticklabels(list(order2), fontsize=9.5)
ax.set_xlabel("treatment effect β (log-odds)")
ax.set_title("v1 → v2 per-decision treatment effect — what changed", fontsize=13, loc="left", pad=12, fontweight="bold")
ax.legend(facecolor=GROUND, edgecolor=RULE, labelcolor=TEXT, fontsize=10, loc="lower right")
for sp in ("top", "right", "left"):
    ax.spines[sp].set_visible(False)
ax.tick_params(length=0); ax.grid(axis="x", alpha=.25)
fig.tight_layout(); fig.savefig(FIGS / "fig6_v1_v2.png", dpi=150, bbox_inches="tight"); plt.close(fig)
print("wrote fig6_v1_v2.png")

# note: fig5 (model x instruction lift heatmap) is generated by scratch/deep_dive.py
# (it needs the raw v5 scores, not just the saved traces).
