...
long conversations. This holds even when the system prompt keeps its other
instructions fixed, so the loss has no visible trigger. Our central finding
is that they do not forget these preferences equally: retention is markedly
uneven from one instruction to the next, and treating it as uniform hides
that variation. Prior work reports mean compliance and mitigates
uniformly, restating every instruction each turn, which assumes all
instructions are retained alike; we measure retention per instruction
instead. We plant
preferences in realistic multi-turn coding sessions on three open-source
Python codebases, across several models and twelve instruction types, and
grade compliance with deterministic checkers rather than an LLM judge, so
the scores are reproducible. A Bayesian hierarchical model then estimates
how strongly each instruction is retained. Most preferences are already
followed by default, a few are honored only when explicitly restated, and
one shows a small negative estimate that is likely a measurement artifact.
Only three of twelve
instruction types gain enough from restating to justify it, which argues
for targeted rather than blanket reinforcement. The asymmetry matters most
for agents deployed in long-running loops, where no human reviews each step:
guardrails are themselves instructions, so a safety-relevant rule can decay
silently while the agent keeps acting, and without per-instruction
monitoring nothing flags it.
...
\begin{abstract}
Los grandes modelos de lenguaje pierden adherencia a las preferencias del
usuario en conversaciones extensas. Esto ocurre incluso cuando el system
prompt mantiene fijas sus dem\'as instrucciones, de modo que la p\'erdida no
tiene se\~nal visible. Nuestro
hallazgo central es que no se olvidan todas las preferencias por igual: la
retenci\'on es marcadamente despareja de una instrucci\'on a otra, y
tratarla como uniforme oculta esa disparidad. El trabajo previo
reporta cumplimiento promedio y mitiga uniformemente, repitiendo cada
instrucci\'on en cada turno, asumiendo que todas se retienen igual;
nosotros medimos la retenci\'on por instrucci\'on. Plantamos preferencias
en sesiones de programaci\'on multi-turno sobre tres repositorios de Python
de c\'odigo abierto, en varios modelos y doce tipos de instrucci\'on, y
evaluamos el cumplimiento con verificadores determin\'isticos en lugar de un
juez LLM, de modo que los puntajes son reproducibles. Un modelo jer\'arquico
bayesiano estima cu\'anto se retiene cada instrucci\'on. La mayor\'ia de las preferencias ya se cumplen por defecto,
unas pocas se respetan solo al enunciarse, y una muestra un peque\~no
efecto negativo, probablemente un artefacto de medici\'on. Solo tres de
doce tipos de instrucci\'on
ganan lo suficiente al reenunciarse para justificarlo, lo que favorece
un refuerzo dirigido antes que uniforme. La asimetr\'ia importa sobre todo
para agentes desplegados en loops, donde ning\'un humano
revisa cada paso: las barreras de seguridad son tambi\'en instrucciones, de
modo que una regla relevante para la seguridad puede degradarse en silencio
mientras el agente sigue actuando, y sin monitoreo por instrucci\'on esa
degradaci\'on pasa inadvertida.
...
irreversible \parencite{mu2025}. Repeating every instruction each turn,
...
based on a sequence of observed responses. We adapt this: the LLM is
...
rates or determine at which turn compliance begins to drop. Fitting actual forgetting curves would
require compliance measurements at every turn, which would then require
...
(Claude) in order to improve the language and readability of the manuscript.
