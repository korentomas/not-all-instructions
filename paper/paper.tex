...
The practical payoff is concrete: of 12 instruction types only 3 show a
reinforcement gain whose interval excludes zero in the full model (two when
restricted to the single model with matched baseline), so a uniform policy
spends most of its tokens where they buy nothing. The retention
probabilities estimated here are the input a Bayesian Knowledge Tracing
system would need; we leave building it to future work.
...
(Claude) to improve the language and readability of the manuscript. All
experimental results were produced by the deterministic checkers and
statistical models described above and were verified by the author, who
