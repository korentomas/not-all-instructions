...
it, so blanket reinforcement spends most of its tokens for no gain. The unevenness cuts the
other way for safety: a guardrail is just another instruction, and the rules
a model would not follow on its own are the ones most likely to slip. Since
safety is conjunctive, an aggregate compliance rate cannot tell whether the
rule that matters still holds, and in an agent loop nothing flags the lapse
without per-instruction monitoring.
...
reenunciarse para justificarlo, de modo que el refuerzo uniforme gasta la
mayor\'ia de sus tokens sin ganancia. La disparidad juega en contra de la
seguridad: una
barrera de seguridad es una instrucci\'on m\'as, y las reglas que el modelo
no seguir\'ia por s\'i solo son las m\'as propensas a fallar. Como la
...
posteriors concentrated near zero and 94\% intervals spanning zero
($\sigma_\text{model}, \sigma_\text{codebase} \approx 0.3$), so these
terms add no explanatory power. We therefore report the simpler
...
($P(\beta < 0) = 0.98$) is the only negative effect in the study. The instruction
...
while varying the negation frame. A reviewer suggested exactly this test;
...
reinforcement gain whose interval excludes zero, so a uniform policy
