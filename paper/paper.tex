...
in loops, call tools, and act over hundreds of turns with no human
reviewing each step. An agent is governed by its instructions: the
...
agent keeps acting through tools whose effects are real and often
...
because baseline data comes from a single model, we read this as
model-invariance within our design rather than an established absence of
model effects. A Bayesian model can then identify which instructions to
...
(Section~\ref{sec:limitations}), so we treat this as a candidate
measurement artifact rather than a robust harm. Isolating the negation
...
effect. This ranking is a cost criterion, not a safety one: a low gain may
...
guardrail fragility: real guardrails often sit at position zero in the
...
beats uniform under a fixed token budget, and real coding-session traces
would test whether the structure holds outside the harness.
...
no detectable effect at this resolution rather than established invariance.
...
Nacional de San Mart\'in, between whose halls I still live; and all my
